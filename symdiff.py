...
def symbolic_diff(f):
    ...
    return sympy.lambdify(syms, d,
                          "math")  # turn SymPy expression into Python lambda function
...
def h(x):
    return tanh(
        x)  # this only works because SymPy has a tanh function too; math.tanh(x) fails
